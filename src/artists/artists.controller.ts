...
export class ArtistsController {
    ...
    @Get(':id')
    findOne(@Res({ passthrough: true }) res: Response, @Param('id') id: string): ArtistDto | string {
        if (!validate(id)) {
    ...
    @Post()
    create(@Res({ passthrough: true }) res: Response, @Body() createArtist: CreateArtistDto): ArtistDto | string {
    ...
    @Put(':id')
    update(@Res({ passthrough: true }) res: Response, @Param('id') id: string, @Body() newArtistData: UpdateArtistDto): ArtistDto | string {
        if (!validate(id)) {
            res.status(HttpStatus.BAD_REQUEST);
            return 'Id not valid';
        };

        if (newArtistData.grammy === undefined || typeof newArtistData.name !== 'string') {
            res.status(HttpStatus.BAD_REQUEST);
